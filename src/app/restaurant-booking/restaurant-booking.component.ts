...
FormSubmit(FormValue:NgForm)
{
  ...
  this.api.getvalue("95beac924a15b72af13226cf037ce8ba").subscribe((res:any)=>{
...
roomvalue(event:any) {
  ...
  this.api.Roomnum(event.target.value).subscribe((res:any)=>{
    ...
    // for(var number of this.findroom){
    //   var totlen = res.docs.length;
     
    //   for(var i=0; i<=totlen; i++)
    //   {
    //     console.log("number",number[i].roomnumber)
    //     this.assignroom[i] = number[i].roomnumber;
    //     // console.log(this.assignroom);
